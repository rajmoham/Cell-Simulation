...
public class FieldStats {
    ...
    public void incrementDiseased(Class cellClass) {
        Counter count = counters.get(cellClass);
        count.incrementDiseased();
    }
        ...
        for (int row = 0; row < field.getDepth(); row++) {
            for (int col = 0; col < field.getWidth(); col++) {
                ...
                if (cell != null) {
                    ...
                    if (cell.getDiseased()) {
                        incrementDiseased(cell.getClass());
                    }
                }
            }
        }
    ...
    /**
     * Returns the percentage of Mycoplasma that is diseased
     * @return the percentage of the Mycoplasma cell that are diseased as an integer
     */
    public int getDiseasedPercentage() {
        int totalCount = 0;
        int diseasedCount = 0;

        for (Class key : counters.keySet()) {
            Counter info = counters.get(key);
            if (info.getName().equals("Mycoplasma")) {
                totalCount = info.getCount();
                diseasedCount = info.getDiseasedCount();
                break;
            }
        }
        ...
        int diseasedPercentage = (int)(100.0 * diseasedCount / totalCount);
